...
class JigsawDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        return {'original': self.images[index], 'index': index}
...
class Memory(object):
    def __init__(self, device, size=2000, weight=0.5):
        self.memory = np.zeros((size, 512))
        self.weighted_sum = np.zeros((size, 512))
        self.weighted_count = 0
        self.weight = weight
        self.device = device
        self.size = size

    def initialize(self, net_func, train_loader):
        ...
        for step, batch in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                index = batch['index']
                output = net_func(images)
                self.weighted_sum[index, :] = np.squeeze(output.cpu().numpy())
